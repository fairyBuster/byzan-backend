...
class Course(models.Model):
    ...
    instructor    = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='courses_instructed')
...
class Transaction(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    provider  = models.CharField(max_length=20, choices=PROVIDER_CHOICES, default='balance')
...
class CourseCertificate(models.Model):
    code = models.CharField(max_length=64, unique=True, default=generate_certificate_code)
    certificate_number = models.CharField(max_length=32, unique=True, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.issued_at and not self.certificate_number and self.pk:
            ...
            type(self).objects.filter(pk=self.pk).update(certificate_number=certificate_number)
...
class LessonQuestion(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='lesson_questions')
    lesson = models.ForeignKey(Lesson, on_delete=models.CASCADE, related_name='questions')
    question = models.TextField()
    answer = models.TextField(blank=True)
    answered_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='lesson_answers')
